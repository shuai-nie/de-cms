...
function InitPage()
{
    (function(e){ TestHasTitle(e); })();

    var selsource = $Obj('selsource');
    var selwriter = $Obj('selwriter');
    var titlechange = $Obj('title');
    var needwatermarkupchange = $Obj('needwatermarkup');
    var needwatermarkdownchange = $Obj('needwatermarkdown');
    ...
    if(needwatermarkupchange){ needwatermarkupchange.onchange=function(e){ TestHasTitle(e); } }
    if(needwatermarkdownchange){ needwatermarkdownchange.onchange=function(e){ TestHasTitle(e); } }
...
function TestHasTitle(e)
{
    ...
    if (document.getElementsByName("id").length > 0) {
        aid = document.getElementsByName("id")[0].value;
    }

    if ($Obj('needwatermarkup') && $Obj('needwatermarkdown')) {
        LoadNewDiv2(e,'article_test_title.php?t='+$Obj('title').value+"&aid="+aid+"&needwatermarkup="+$Obj('needwatermarkup').checked+"&needwatermarkdown="+$Obj('needwatermarkdown').checked,'mytitle',"dlgTesttitle");
    } else {
        LoadNewDiv2(e,'article_test_title.php?t='+$Obj('title').value+"&aid="+aid,'mytitle',"dlgTesttitle");
    }
}
